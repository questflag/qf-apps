...
var infraServices = builder.AddProject<Projects.QuestFlag_Infrastructure_Services>("service-qf-infra")
    ...
    .WithEnvironment("Storage__MinioEndpoint", Compose("STORAGE_MINIO_ENDPOINT", "infra-minio:9000"))
    .WithEnvironment("Kafka__BootstrapServers", Compose("KAFKA_BOOTSTRAP_SERVERS", "infra-kafka:9092"))
    .WithEnvironment("ConnectionStrings__InfrastructureConnection", Compose("INFRASTRUCTURE_CONNECTION_STRING", "Host=infra-postgres;Port=5432;Database=questflag_infrastructure;Username=postgres;Password=P@ssw0rd!Qf2026!"))
...
var passportServices = builder.AddProject<Projects.QuestFlag_Passport_Services>("questflag-passport-services")
    ...
    .WithEnvironment("ConnectionStrings__PassportConnection", Compose("PASSPORT_CONNECTION_STRING", "Host=infra-postgres;Port=5432;Database=QuestFlag_Passport;Username=postgres;Password=P@ssw0rd!Qf2026!"))
    .WithEnvironment("ServiceUrls__InfraWebApp", urlInfraWebApp)
    .WithEnvironment("ServiceUrls__InfraWebAppHttp", $"http://localhost:{infraWebAppHttpPort}")
